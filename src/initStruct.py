...
##########################################################################		
	...
	def randomize(self):
		...
		for i in xrange(0, 4):
			...
			self.C_PLANES_PER_LAYER = random.choice(self.PLANE_RANGE[i])
		...
		# Q 
		self.Q[0] = random.randrange(.05, .75)
		for i in xrange(0, 4):
			self.Q[i+1] = random.choice(self.Q_RANGE[i])

		# R 
		self.R[0] = random.choice(range(2, 6))
		self.R[1] = random.randrange(.1, .9)*2
		self.R[2] = max(random.randrange(.1, .9)*2, self.R[1])
		self.R[3] = max(random.randrange(.1, .9)*2, self.R[1])

		# C 
		self.C[0] = random.randrange(.4, .8)
		self.C[1] = random.randrange(.09 , (1.0 - self.C[0])/2)
		self.C[2] = random.randrange(.05, (1.0 - self.C[0] - self.C[1])/2)
		self.C[3] = (1.0 - self.C[0] - self.C[1] - self.C[2])/2

		# D 
		self.D[0] = random.randrange(.4, .8)
		self.D[1] = random.randrange(.09 , (1.0 - self.D[0])/2)
		self.D[2] = random.randrange(.05, (1.0 - self.D[0] - self.D[1])/2)
